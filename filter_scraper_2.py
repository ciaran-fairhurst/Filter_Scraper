...
page = requests.get(url_to_scrape).text.encode('utf8').split('gname=')[1:]
...
for ins in instruments:#[-2:]:
    ...
    page = requests.get(url_to_scrape_2).text.encode('utf8')
    ...
    if cameras != []:
        for cam in cameras:
            ...
            page = requests.get(url_to_scrape_3).text.encode('utf8')
...
for i,filt in enumerate(list_of_filters):
    ...
    if not os.path.exists( path+'/'+filename+".txt" ):
        urllib.urlretrieve ("http://svo2.cab.inta-csic.es/svo/theory/fps/getdata.php?format=ascii&id="+filt, path+'/'+filename+".txt")
